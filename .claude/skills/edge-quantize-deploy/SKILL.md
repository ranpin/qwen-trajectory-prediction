...
## 量化 lm_head 需要多大的卡（2×T4 做不到，实测 5 次）

`--lm_head_quantization int4_awq` 看着只是加个参数，但 lm_head（vocab×hidden，如 4096×151936 = 622 M）
fake-quantize 需要约 **1.16 GiB 连续临时张量**，落在**承载该层的那一张卡**上（跨卡借不到）。在 2×T4（14.56 GiB/卡）上：

| max_memory | 实测 placement | 结果 |
|---|---|---|
| 未设 / 11 GiB/卡 / 10 GiB/卡 | 全 GPU | **OOM**（该卡仅剩 ~1.0 GiB） |
| 9 GiB/卡 / 11+6 非对称 | 有模块→CPU | **AWQ 校准 + accelerate offload hook 不兼容 → illegal memory access** |

**`max_memory` 只约束初始权重放置**：两个不同上限 OOM 时**剩余显存完全相同**，量化状态会把显存吃回同一水位。
⇒ 压总量不是有效杠杆。**要做 lm_head 量化就用单卡 ≥24 GB（L4/A6000/A100）**，让权重+临时张量无需分片。

**通用规律**：`量化/导出/建引擎` 这类**逐层处理**的流程，峰值内存由**最大单层**决定，
而词表投影层通常就是最大单层（比骨干最大层大一个数量级）⇒ 它既是**最大的优化机会**（占 decode 字节 25%+），
也是**最先爆内存的地方**。算显存只算总权重，必然低估。

## 模型规模 vs 精度：先量化，别急着换小模型

同一基准(n=210)实测：8B 上 FP16→INT8→INT4 正确率 76.2→75.2→73.8，**McNemar 全部 p>0.4（不显著）**；
但 INT4 下 8B→2B 掉到 65.2%，**p=0.005 显著**。⇒ **激进量化几乎免费，缩小模型有可测代价**。
另：**小模型的带宽效率更低**（2B 只到可达带宽 67%，8B INT4 到 97%），所以"按字节比估加速"在小模型上会明显高估，
只能当上界用。
