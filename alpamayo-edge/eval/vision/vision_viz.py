#!/usr/bin/env python3
"""Vision-encoder cost and multi-camera scaling (INT4 LLM + fp16 vision encoder).
...
Out:  alpamayo-edge/docs/figures/vision_encoder.png
"""
import json
import os
import sys
...
x = np.arange(len(f))
b1 = a1.bar(x, vis, 0.55, label="vision encoder (fp16)", color=C8, **bar_kw())
...
a1.set_title("TTFT split by stage (INT4 LLM, 448x256 frames)")
...
a2.annotate(f"engine maxInputLen={MAX_INPUT_LEN}\n=> {int(max_frames)} frames at "
            f"{TOK_PER_FRAME} tok/frame",
...
a2.set_ylabel("vision encoder GPU time (ms)")
a2.set_title("Vision encoder GPU time vs input frames")
...
fig.suptitle(f"Vision encoder cost, {TOK_PER_FRAME} tokens/frame (448x256 after resize) "
             "(orin-dog, INT4 LLM + fp16 vision encoder, batch = 1)")
fig.tight_layout(rect=(0, 0.06, 1, 0.94))
fig.savefig(os.path.join(OUT, "vision_encoder.png")); plt.close(fig)
...
print("wrote", os.path.join(OUT, "vision_encoder.png"))
