...
note(13.2, "F", "§1.5 发现 → 优化 A（预测 +23%，至今未验证）",
     "AWQ 默认跳过输出层 ⇒ 它独占 1.245 GB / token 的搬运量、22.0% 的 decode 时间。\n"
     "量化掉它可把每 token 字节 4.853→3.932 GB（−19%），预测 TPOT 33.1→26.8 ms。\n"
     "5 次云端尝试全部失败（T4×2 显存两面夹击）⇒ 仍是未验证的预测，见 §1.8 末。", F_E)
...
budget = [
    ...
    ("lm_head · fp16 未量化", "1.245 GB", "25.6%", "22.0%", F_E),
]
