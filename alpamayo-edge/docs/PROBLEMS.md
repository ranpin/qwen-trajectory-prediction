...
- **解决**：帧**长边缩到 448px**；实测（2026-07-26 复核）每帧 patch 数为 **448**（448×252→448×256，28×16）或 **616**（448×358→448×352，28×22），6 帧 = 2688 / 3696 < 4096 ⇒ 通过，但 5:4 那批已用掉 90% 的 patch 预算。**FP16 与 Orin 用同一批缩放帧**保证掉点可比。（此前本条写的"每帧 ~256 patch"是估算，实测已修正。）
